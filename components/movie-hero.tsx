...
export const MovieHero: React.FC<MovieHeroProps> = ({ movies, count = 1 }) => {
  ...
  if (data.length === 0)
    return <Skeleton className="h-hero relative w-full rounded-lg" />;

  return data.map((item) => (
    <div
      className="w-full rounded-lg drop-shadow-lg overflow-hidden relative h-hero"
      key={item.id}
    >
      ...
      <MediaBackdrop priority image={item.backdrop_path} alt={item.title} />
